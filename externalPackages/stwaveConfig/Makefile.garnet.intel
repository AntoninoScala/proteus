...
ifneq ($(TARG),)
  ...
  STWAVE        := $(TARG)stwave
  STWAVE_P      := $(TARG)stwave_p
  STWAVE_EP     := $(TARG)stwave_ep
endif

EXEC    := $(STWAVE) $(STWAVE_P) $(STWAVE_EP) 
...
# Removed MAKELEVEL==0 if-statement -- Matt M.
#if $(MAKELEVEL,0)
#$(STWAVE):
#	$(MAKE) BUILDTYPE=$(STWAVE)  delete_build_files $@
#
#$(STWAVE_P):
#	$(MAKE) BUILDTYPE=$(STWAVE_P) delete_build_files $@
#
#$(STWAVE_EP):
#	$(MAKE) BUILDTYPE=$(STWAVE_EP) delete_build_files $@
#
#$(LIBSTW):
#	@echo $(MAKE) LIBSTW=$@ BUILDTYPE=$(LIBSTW) delete_build_files $@
#	$(MAKE) LIBSTW=$@ BUILDTYPE=$(LIBSTW) delete_build_files $@
#else
